    ...
    try {
        ...
        foreach ($detalles as $detalle) {
            ...
            if ($itemType === 'servicio') {
                // Para servicios: usar cuenta_ingresos
                $cuentaIngresos = $detalle['cuenta_ingresos'] ?? null;
                if ($cuentaIngresos) {
                    // Agrupar ingresos por cuenta (total sin descuento para balancear correctamente)
                    if (!isset($ingresosPorCuenta[$cuentaIngresos])) {
                        $ingresosPorCuenta[$cuentaIngresos] = 0;
                    }
                    $ingresosPorCuenta[$cuentaIngresos] += $totalItem + $descuentoItem; // Total sin descuento
        ...
        foreach ($facturasCobradas as $factura) {
            ...
            foreach ($detalles as $detalle) {
                ...
                if ($itemType === 'servicio') {
                    ...
                    if ($cuentaIngresos) {
                        // Agrupar ingresos por cuenta (total sin descuento para balancear correctamente)
                        // Los ingresos se registran como el total sin descuento (totalItem + descuentoItem)
                        if (!isset($ingresosPorCuenta[$cuentaIngresos])) {
                            $ingresosPorCuenta[$cuentaIngresos] = 0;
                        }
                        $ingresosPorCuenta[$cuentaIngresos] += $totalItem + $descuentoItem; // Total sin descuento
