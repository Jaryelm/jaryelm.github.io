...
$turno = isset($_POST['turno']) ? trim($_POST['turno']) : '';
	...
	if ($turnoActivo > 0) {
		echo '<script>
			swal("Información", "Ya tiene un turno iniciado sin cerrar. Debe cerrar el turno actual antes de iniciar uno nuevo.", "info");
		</script>';
		exit;
	}
	...
	if ($insertTurno->execute([
		...
		echo '<script>
			swal("¡Turno Iniciado!", "Turno iniciado correctamente.\\nFecha: ' . $fechaActual . '", "success")
			.then(function() {
		...
		$connect->rollBack();
		echo '<script>
			swal("Error", "Error al registrar el inicio de turno.", "error");
		</script>';
	}
