...
class ZKTeco
{
    public $_ip;
    public $_port;
    /** Destino UDP fijo; no pasar a socket_recvfrom (PHP sobrescribe refs con el origen). */
    public $_device_ip;
    public $_device_port;
    public $_zkclient;
 ...
 */
    public function __construct($ip, $port = 4370)
    {
        ...
        $this->_ip = $ip;
        $this->_port = $port;
        $this->_device_ip = (string) $ip;
        $this->_device_port = (int) $port;
    ...
    public function _command($command, $command_string, $type = Util::COMMAND_TYPE_GENERAL)
    {
        ...
        socket_sendto($this->_zkclient, $buf, strlen($buf), 0, $this->_device_ip, $this->_device_port);

        try {
            $udpFrom = '';
            $udpPort = 0;
            @socket_recvfrom($this->_zkclient, $this->_data_recv, 1024, 0, $udpFrom, $udpPort);
