...
// Consultar registros de PROCEDIMIENTOS
$stmtData = $connect->prepare("
    SELECT turno, procedimiento_realizado, procesado_por, fecha_registro
    FROM procedimientos
    WHERE idpa = :idpa 
...
if (empty($data)) {
    die('Error: No se encontraron registros de procedimientos.');
}
...
// Título de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 15, mb_convert_encoding(mb_strtoupper('REGISTRO DE PROCEDIMIENTOS', 'UTF-8'), 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');

// Encabezado de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(240, 240, 240);
$columns = ['TURNO', 'PROCEDIMIENTO REALIZADO', 'PROCESADO POR', 'FECHA'];
$columnWidths = [30, 80, 50, 34];
...
$pdf->SetFont('Arial', '', 9);
foreach ($data as $row) {
    $pdf->Cell($columnWidths[0], 6, mb_convert_encoding($row['turno'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[1], 6, mb_convert_encoding($row['procedimiento_realizado'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[2], 6, mb_convert_encoding($row['procesado_por'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[3], 6, mb_convert_encoding($row['fecha_registro'], 'ISO-8859-1'), 1);
    $pdf->Ln();
}

// Salida del PDF
$pdf->Output('D', 'Procedimientos.pdf');
?>
