...
<form action="" enctype="multipart/form-data" method="POST" autocomplete="off" onsubmit="return validacion()">
    <div class="containerss">
        ...
        <select id="nomcl_dynamic" name="nomcl_dynamic" class="select2" style="width: 100%;" placeholder="Escribe o selecciona un paciente" onchange="handleDynamicSelection()">
            ...
            <?php
            // Pacientes registrados (formulario completo)
            $query = $connect->prepare("SELECT CONCAT(nompa, ' ', apepa) AS full_name, numhs, cump FROM patients");
            $query->execute();
            while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
                $cump_val = $row['cump'] ?? '';
                echo '<option value="' . htmlspecialchars($row['full_name']) . '" data-dni="' . htmlspecialchars($row['numhs']) . '" data-cump="' . htmlspecialchars($cump_val) . '">' . htmlspecialchars($row['full_name']) . '</option>';
            }
            // Pacientes ambulatorios (registrados desde checkout)
            $stmt_amb = $connect->query("SHOW TABLES LIKE 'patients_ambulatorios'");
            if ($stmt_amb->rowCount() > 0) {
                $query_amb = $connect->prepare("SELECT CONCAT(TRIM(nompa), ' ', TRIM(IFNULL(apepa,''))) AS full_name, numhs, cump FROM patients_ambulatorios ORDER BY fecha_registro DESC");
                $query_amb->execute();
                while ($row = $query_amb->fetch(PDO::FETCH_ASSOC)) {
                    $full = trim($row['full_name']);
                    if ($full === '' || $full === '-') { $full = trim(str_replace(' -', '', $row['full_name'])); }
                    $cump_val = $row['cump'] ?? '';
                    echo '<option value="' . htmlspecialchars($full) . '" data-dni="' . htmlspecialchars($row['numhs']) . '" data-cump="' . htmlspecialchars($cump_val) . '">' . htmlspecialchars($full) . '</option>';
                }
            }
            ?>
        </select>
...
<input type="hidden" name="dni_paciente" id="dni_paciente" value="">
        ...
        <div id="tarjeta_fields" style="display: none; margin-top: 20px; margin-bottom: 20px; padding: 20px; border: 2px solid #06adbf; border-radius: 8px; background-color: #f0f9fa;">
            ...
            <select name="banco_emisor" id="banco_emisor" style="margin-bottom: 15px;">
                ...
                <?php
                // Obtener cuentas de tarjetas del catálogo (códigos que empiezan con 11010040)
                try {
                    $stmt_tarjetas = $connect->prepare("
                        SELECT cuenta, nombre 
                        FROM cuentas_catalogo 
                        WHERE cuenta LIKE '11010040%' 
                        ORDER BY cuenta ASC
                    ");
                    $stmt_tarjetas->execute();
                    $cuentas_tarjetas = $stmt_tarjetas->fetchAll(PDO::FETCH_ASSOC);
                    
                    foreach ($cuentas_tarjetas as $cuenta_tarjeta) {
                        $codigo_cuenta = trim($cuenta_tarjeta['cuenta']); // Eliminar espacios
                        $nombre_cuenta = htmlspecialchars($cuenta_tarjeta['nombre']);
                        echo "<option value=\"$codigo_cuenta\">$nombre_cuenta</option>";
                    }
                } catch (PDOException $e) {
                    error_log("Error al obtener cuentas de tarjetas: " . $e->getMessage());
                    // Fallback a opciones por defecto si hay error
                    echo "<option value=\"110100401\">Tarjetas de Credito Bac</option>";
                    echo "<option value=\"110100402\">Tarjetas de Credito Banpais</option>";
                }
                ?>
            </select>
