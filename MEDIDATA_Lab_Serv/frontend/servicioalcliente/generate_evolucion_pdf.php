...
// Consultar registros de la "Hoja de Evolución"
$stmtData = $connect->prepare("
    SELECT nota_evolucion, fecha_hora, ordenes_medicas, procesado_por 
...
if (empty($data)) {
    die('Error: No se encontraron registros para la Hoja de Evolución.');
}
...
// Título de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 15, mb_convert_encoding(mb_strtoupper('REGISTROS DE EVOLUCIÓN', 'UTF-8'), 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');

// Encabezado de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(240, 240, 240);
$columns = ['NOTA DE EVOLUCIÓN', 'FECHA Y HORA', 'ÓRDENES MÉDICAS', 'PROCESADO POR'];
$columnWidths = [50, 45, 50, 50];
...
$pdf->SetFont('Arial', '', 9);
foreach ($data as $row) {
    $pdf->Cell($columnWidths[0], 6, mb_convert_encoding($row['nota_evolucion'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[1], 6, mb_convert_encoding($row['fecha_hora'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[2], 6, mb_convert_encoding($row['ordenes_medicas'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[3], 6, mb_convert_encoding($row['procesado_por'], 'ISO-8859-1'), 1);
    $pdf->Ln();
}

// Salida del PDF
$pdf->Output('D', 'Hoja Evolucion.pdf');
?>
