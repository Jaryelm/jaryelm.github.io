...
// Clase para agregar el Footer personalizado
...
// Configurar zona horaria y validación
...
// Consultar registros de "Excretas"
...
if (empty($data)) {
    die('Error: No se encontraron registros para la Hoja de Excretas.');
}
...
// Título de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 15, mb_convert_encoding(mb_strtoupper('REGISTROS DE EXCRETAS', 'UTF-8'), 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');

// Encabezado de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(240, 240, 240);
$columns = ['FECHA', 'HORA', 'ORINA', 'VÓMITO', 'DRENAJE', 'SUCCIÓN', 'OTROS', 'PROCESADO POR'];
$columnWidths = [25, 20, 20, 20, 25, 20, 30, 35];
...
$pdf->SetFont('Arial', '', 9);
foreach ($data as $row) {
    $pdf->Cell($columnWidths[0], 6, mb_convert_encoding($row['fecha'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[1], 6, mb_convert_encoding($row['hora'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[2], 6, mb_convert_encoding($row['orina'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[3], 6, mb_convert_encoding($row['vomito'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[4], 6, mb_convert_encoding($row['drenaje'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[5], 6, mb_convert_encoding($row['succion'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[6], 6, mb_convert_encoding($row['otros'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[7], 6, mb_convert_encoding($row['procesado_por'], 'ISO-8859-1'), 1);
    $pdf->Ln();
}

// Salida del PDF
$pdf->Output('D', 'Hoja Excretas.pdf');
?>