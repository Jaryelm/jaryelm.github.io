            ...
            function loadCollaborators() {
                $.ajax({
                    ...
                    success: function(collaborators) {
                        ...
                        tbody.empty();
                        $('#colaboradores_head').html(RRHH_COLAB.headRow());

                        if (!Array.isArray(collaborators)) {
                            tbody.append('<tr><td colspan="' + RRHH_COLAB.colCount() + '">No se pudieron cargar los colaboradores.</td></tr>');
                            return;
                        }
                        ...
                        const colaboradoresTable = $('#colaboradores_table').DataTable({
                            destroy: true,
                            pageLength: 10,
                            dom: 'Bfrtip',
                        ...
                        function ajustarColumnasColaboradores() {
                            colaboradoresTable.columns.adjust();
                        }
                        $('.toggle-sidebar').off('click.colaboradores').on('click.colaboradores', function () {
                            setTimeout(ajustarColumnasColaboradores, 350);
                        });
                        $(window).off('resize.colaboradores').on('resize.colaboradores', ajustarColumnasColaboradores);
                    },
