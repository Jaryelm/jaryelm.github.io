                                ...
                                <tbody>
                                    <?php foreach($data as $d): ?>
                                        <tr>
                                            ...
                                            <td data-title="Estado" class="rrhh-col-center">
                                                <label class="switch">
                                                    <input type="checkbox" class="state-toggle" data-id="<?php echo $d->id; ?>" <?php echo (isset($d->state) && $d->state == 0) ? '' : 'checked'; ?>>
                                                    <span class="slider"></span>
                                                </label>
                                            </td>
                                            <td data-title="Acciones" class="rrhh-col-center">
                                                <span class="rrhh-table-actions">
                                                    <a title="Editar" href="javascript:void(0)" class="fa fa-edit rrhh-icon-edit btn-edit-position" data-id="<?php echo (int) $d->id; ?>" data-name="<?php echo htmlspecialchars((string) $d->name, ENT_QUOTES, 'UTF-8'); ?>"></a>
                                                </span>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
...
<?php include __DIR__ . '/_rrhh_select2_foot.php'; ?>
    ...
    $(document).ready(function() {
        $('#positionsTable').DataTable({
            ...
            autoWidth: false,
            columnDefs: [
                { targets: [1, 2], className: 'rrhh-col-center', orderable: false }
            ],
            language: {
    ...
    function deletePosition(id) {
        Swal.fire({
            title: '¿Estás seguro?',
            text: 'Esta acción eliminará permanentemente la posición. Para ocultarla temporalmente, use el interruptor de estado.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '../../backend/php/delete_position.php',
                    data: { id: id },
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
