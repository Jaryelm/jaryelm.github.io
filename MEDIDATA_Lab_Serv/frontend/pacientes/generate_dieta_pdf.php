...
// Extender la clase FPDF para agregar el Footer personalizado
...
// Consultar registros de la "Hoja de Control de Dieta"
$stmtData = $connect->prepare("
    SELECT fecha, turno, tipo_dieta, procesado_por, created_at 
    FROM control_dieta 
    WHERE idpa = :idpa 
...
if (empty($data)) {
    die('Error: No se encontraron registros para la Hoja de Control de Dieta.');
}
...
// Título de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 15, mb_convert_encoding(mb_strtoupper('REGISTROS DE CONTROL DE DIETA', 'UTF-8'), 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');

// Encabezado de la tabla
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(240, 240, 240);
$columns = ['FECHA', 'TURNO', 'TIPO DE DIETA', 'PROCESADO POR'];
$columnWidths = [30, 30, 80, 55];
...
foreach ($data as $row) {
    ...
    $pdf->Cell($columnWidths[1], 6, mb_convert_encoding($row['turno'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[2], 6, mb_convert_encoding($row['tipo_dieta'], 'ISO-8859-1'), 1);
    $pdf->Cell($columnWidths[3], 6, mb_convert_encoding($row['procesado_por'], 'ISO-8859-1'), 1);
    $pdf->Ln();
}

// Salida del PDF
$pdf->Output('D', 'Hoja Control Dieta.pdf');
?>
