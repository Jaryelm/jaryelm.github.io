...
// Obtener datos de la autorización quirúrgica
$stmtAutoQuiru = $connect->prepare("SELECT intervencion_quirurgica, consistente_en FROM autorizacion_quirurgica WHERE idpa = :idpa");
$stmtAutoQuiru->bindParam(':idpa', $idpa, PDO::PARAM_INT);
$stmtAutoQuiru->execute();
$auto_quiru = $stmtAutoQuiru->fetch(PDO::FETCH_ASSOC);

if (!$auto_quiru) {
    die('Error: No se encontró la autorización quirúrgica.');
}
...
$pdf->Cell(0, 10, mb_convert_encoding('AUTORIZACIÓN QUIRÚRGICA', 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');
$pdf->Ln(10);
...
$pdf->SetFont('Arial', '', 12);
$pdf->Write(8, mb_convert_encoding("Por este medio AUTORIZO al personal Médico Enfermeria y de Apoyo paramédico, para que me practiquen la intervención quirúrgica: ", 'ISO-8859-1', 'UTF-8'));

$pdf->SetFont('Arial', 'B', 12);
$pdf->Write(8, mb_convert_encoding("{$auto_quiru['intervencion_quirurgica']} ", 'ISO-8859-1', 'UTF-8'));

// Declaración legal con motivo en negrita
$pdf->SetFont('Arial', '', 12);
$pdf->Write(8, mb_convert_encoding("Consistente en: ", 'ISO-8859-1', 'UTF-8'));

$pdf->SetFont('Arial', 'B', 12);
$pdf->Write(8, mb_convert_encoding("{$auto_quiru['consistente_en']} ", 'ISO-8859-1', 'UTF-8'));

$pdf->Ln(15);

$pdf->SetFont('Arial', '', 12);
$pdf->Write(8, mb_convert_encoding("Al cual me someto en un acto VOLUNTARIO por lo que doy fé que antes de firmar esta autorización, he sido ORIENTADO E INFORMADO sobre los beneficios y riesgos a los que me someto.", 'ISO-8859-1', 'UTF-8'));
...
$textoFecha = "Para constancia de tal autorización, firme la presente en la ciudad de: Comayaguela, M.D.C. a los {$dia} días del mes de {$mes} del {$anio}.";
...
$pdf->Output('D', 'Autorizacion Quirurgica.pdf');
