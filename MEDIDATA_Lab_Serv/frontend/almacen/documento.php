<?php
/* Inicializar la sesión para acceder a los datos del usuario */
session_start();
...
try {
    ...
    error_log('documento.php CAI dinámico: ' . $e->getMessage());
}
...
class PDF_MC extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 5, mb_convert_encoding('MEDICASA S.A DE C.V', 'ISO-8859-1', 'UTF-8'), 0, 1, 'L');
        $this->SetXY(65, 20); 
...
$pdf->Cell(0, 5, mb_convert_encoding('MEDICASA S.A DE C.V', 'ISO-8859-1', 'UTF-8'), 0, 1, 'L');
$pdf->SetXY(65, 20); 
...
SELECT 
    ...
    od.other_discount,
    CASE 
        WHEN od.item_type = 'producto' AND od.hospitalario_id IS NOT NULL THEN ah.nompro
        WHEN od.item_type = 'producto' THEN p.nompro 
        WHEN od.item_type = 'servicio' THEN s.nomservicio 
    END AS descripcion,
    CASE 
...
// Teléfono del paciente: usar el guardado en la orden (ambulatorio manual) o buscar en patients
$telefonoPaciente = !empty($order[0]['telefono_paciente']) ? $order[0]['telefono_paciente'] : null;
...
$fields = [
    ...
    'TIPO' => $order[0]['tipo'],
    'MEDICO ATENCIÓN' => $order[0]['remitente'] ?? 'N/A',
    'REMITE' => $order[0]['remite'] ?? 'N/A',
    'TELEFONO REMITE' => $order[0]['telefono_remite'] ?? 'N/A',
...
/* Encabezados de la tabla: Descripción más ancha; Cantidad, Precio, Descuento y Total reducidos */
$headers = ['Descripción', 'Cantidad', 'Precio Unitario', 'Descuento', 'Total'];
$anchoContenido = $colWidth * 3; // 291 mm - igual que bloque de datos y totales
$colWidths = [
    167,  // Descripción (más espacio para textos largos)
    18,   // Cantidad
    38,   // Precio Unitario
    32,   // Descuento
    36    // Total (291 total)
...
foreach ($order as $item) {
    // Verificar si es "PLACA DE RAYOS X" y excluirla de la factura
    $descripcion = $item['descripcion'] ?? '';
    $es_placa = (stripos($descripcion, 'PLACA') !== false && stripos($descripcion, 'RAYOS') !== false);
    
    // Saltar la placa, no debe aparecer en la factura
    if ($es_placa) {
        continue;
    }
    ...
    // Mostrar la fila del ítem en la tabla del PDF (sin Código ni Impuesto)
    $fila = [
        $item['descripcion'], // Descripción
        $item['cantidad'], // Cantidad
        'LPS. ' . number_format($item['precio_unitario'], 2), // Precio unitario
        'LPS. ' . number_format($total_descuentos_item, 2), // Descuento
        'LPS. ' . number_format($total_after_discount, 2) // Total
    ];
...
if ($signatureBlob) {
    ...
    if (@getimagesize($tempImagePath) !== false) {
        ...
        // Si GD no está disponible, usar la imagen directamente sin procesamiento
...
/* Salida del PDF */
// Si viene el parámetro 'view=inline', mostrar en línea; si no, descargar
$output_mode = (isset($_GET['view']) && $_GET['view'] === 'inline') ? 'I' : 'D';
