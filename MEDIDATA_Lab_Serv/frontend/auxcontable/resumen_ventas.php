...
<h1 class="title"><?php echo $saludo . ', <strong>' . $name . '</strong>'; ?></h1>
            ...
            <button class="button" onclick="cambiarColor(this, 'resumen_ventas.php')">Resumen de Ventas</button>
...
<td class="col-acciones">
    <div class="acciones-btns">
        ...
        <!-- Botón Anular Factura deshabilitado para rol Auxiliar Contable -->
    </div>
</td>
            </tr>
        <?php endforeach; ?>
...
<!-- Modal para Anular Factura (Observación) -->
<div id="anulacionModal" class="modal" style="display: none !important;">
    <div class="modal-content modal-anulacion">
        <span class="close-btn" onclick="cerrarModalAnulacion()">&times;</span>
        <h2>Observación</h2>
        <p class="modal-desc">Indique el motivo de la anulación de la factura:</p>
        <div class="form-group-modal">
            <textarea id="observacionAnulacion" class="observacion-input" rows="4" placeholder="Escriba aquí el motivo de la anulación..." maxlength="500"></textarea>
        </div>
        <div class="modal-actions">
            <button type="button" class="btn-modal-cancelar" onclick="cerrarModalAnulacion()">Cancelar</button>
            <button type="button" class="btn-modal-confirmar" onclick="confirmarAnulacion()">Anular Factura</button>
        </div>
    </div>
</div>
...
// Modal de anulación: variables para guardar orderId e invoiceNumber
let anulacionOrderId = null;
let anulacionInvoiceNumber = '';

function abrirModalAnulacion(btn) {
    anulacionOrderId = parseInt(btn.dataset.orderId, 10);
    anulacionInvoiceNumber = btn.dataset.invoiceNumber || '';
    document.getElementById('observacionAnulacion').value = '';
    document.getElementById('anulacionModal').style.display = 'flex';
}

function cerrarModalAnulacion() {
    document.getElementById('anulacionModal').style.display = 'none';
    anulacionOrderId = null;
    anulacionInvoiceNumber = '';
}
...
function confirmarAnulacion() {
    ...
    fetch('../../backend/registros/anular_factura.php', {
        ...
        body: JSON.stringify({ orderId: anulacionOrderId, observacion: observacion })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (devolutionModal) {
        devolutionModal.style.display = "none";
    }
    const anulacionModal = document.getElementById("anulacionModal");
    if (anulacionModal) {
        anulacionModal.style.display = "none";
    }
});
...
window.onclick = function (event) {
    ...
    const anulacionModal = document.getElementById("anulacionModal");
    if (event.target === anulacionModal) {
        cerrarModalAnulacion();
    }
};
