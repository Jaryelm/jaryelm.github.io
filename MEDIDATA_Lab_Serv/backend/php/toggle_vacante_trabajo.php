...
$status = $deleted ? 'Cerrada' : 'Abierta';

try {
    $stmt = $pdo->prepare('UPDATE vacant_positions SET deleted = ?, status = ? WHERE id = ?');
    $ok = $stmt->execute([$deleted, $status, $id]);
    echo json_encode([
        'success' => (bool) $ok,
        'message' => $ok ? 'Estado actualizado' : 'No se pudo actualizar',
    ]);
} catch (Throwable $e) {
    error_log('toggle_vacante_trabajo: ' . $e->getMessage());
