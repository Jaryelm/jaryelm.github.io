...
$idnur = (int) ($_POST['nuridp'] ?? 0);
$numide = strtoupper(trim((string) ($_POST['nuriden'] ?? '')));
$nomnur = strtoupper(trim((string) ($_POST['nurnam'] ?? '')));
$apenur = strtoupper(trim((string) ($_POST['nurape'] ?? '')));
$nacinur = trim((string) ($_POST['nurdat'] ?? ''));
$sexnur = trim((string) ($_POST['nurge'] ?? ''));
...
try {
    ...
    if ($idUser !== null) {
        $linked = medidata_staff_id_user_linked($connect, $idUser, 'nurse', $idnur);
        if ($linked !== null) {
    ...
    $stmtCurrent = $connect->prepare("SELECT numide, id_candidate_rrhh FROM nurse WHERE idnur = ? LIMIT 1");
    $stmtCurrent->execute([$idnur]);
    ...
    $updates = [
        'id_user = :id_user', 'numide = :numide', 'nomnur = :nomnur', 'apenur = :apenur',
        'nacinur = :nacinur', 'sexnur = :sexnur', 'num_empleado = :num_empleado',
    ...
    $sql = 'UPDATE nurse SET ' . implode(', ', $updates) . ' WHERE idnur = :idnur LIMIT 1';
    ...
    error_log('upd_nurse: ' . $e->getMessage());
