...
if (isset($_POST['login'])) {
    ...
    if ($errMsg == '') {
        try {
            $stmt = $connect->prepare('SELECT id, username, name, email, password, rol, state FROM users WHERE username = :username');
            ...
            if ($data == false) {
                ...
                if ($password == $data['password']) {
                    // Bloquear acceso a cuentas desactivadas (state = '0')
                    if (isset($data['state']) && (string) $data['state'] === '0') {
                        $_SESSION['errMsg'] = 'Usted ya no cuenta con acceso, por favor comuniquese con IT';
                        header('Location: ../frontend/login.php');
                        exit();
                    }
