...
$area_colaborador = $_POST['area_colaborador'] ?? '';
...
if ($area_colaborador === '' || $numide === '' || $nombres === '') {
    echo '<script>Swal.fire("Campos requeridos", "Complete el área, identificación y nombres.", "warning");</script>';
    return;
}
...
if ($lista_contexto === 'medicos' && $area_colaborador !== 'doctor') {
    echo '<script>Swal.fire("Área incorrecta", "Desde la lista de médicos solo puede registrar un médico.", "warning");</script>';
    return;
}
if ($lista_contexto === 'colaboradores' && $area_colaborador === 'doctor') {
    echo '<script>Swal.fire("Área incorrecta", "Los médicos se registran desde la lista de médicos.", "warning");</script>';
    return;
}
...
try {
    if ($idUser !== null) {
        $linked = medidata_staff_id_user_linked($connect, $idUser, $area_colaborador);
        if ($linked !== null) {
    ...
    $check = $connect->prepare("SELECT COUNT(*) FROM {$area_colaborador} WHERE {$col_numide} = :numide");
    $check->execute([':numide' => $numide]);
    if ((int) $check->fetchColumn() > 0) {
        echo '<script>Swal.fire("Duplicado", "Ya existe un colaborador con esa identificación en esta área.", "warning");</script>';
        return;
    }

    // Autogenerar num_empleado si está vacío
    if ($num_empleado === '') {
        $stmtC = $connect->query("SELECT COUNT(*) FROM {$area_colaborador}");
        $c = $stmtC->fetchColumn() + 1;
        $num_empleado = 'EMP-' . strtoupper(substr($area_colaborador, 0, 3)) . '-' . str_pad($c, 3, '0', STR_PAD_LEFT);
    }
    ...
    $stmt = $connect->prepare("
        INSERT INTO {$area_colaborador} (
            id_user, {$col_numide}, {$col_nombres}, {$col_apellidos}, {$col_nacimiento}, {$col_genero}, state,
    ...
    $ok = $stmt->execute([
        ...
        ':nombres' => $nombres,
        ':apellidos' => $apellidos,
        ':nacimiento' => $fecha_nacimiento ?: null,
        ':genero' => $genero,
    ...
    error_log('add_colaborador: ' . $e->getMessage());
