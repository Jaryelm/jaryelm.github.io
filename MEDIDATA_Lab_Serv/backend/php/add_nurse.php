...
$numide = strtoupper(trim((string) ($_POST['nuriden'] ?? '')));
$nomadm = strtoupper(trim((string) ($_POST['nurnam'] ?? '')));
$apeadm = strtoupper(trim((string) ($_POST['nurape'] ?? '')));
$nacadm = trim((string) ($_POST['nurdat'] ?? ''));
$sexadm = trim((string) ($_POST['nurge'] ?? ''));
$idUser = medidata_staff_parse_id_user($_POST['nurid_user'] ?? null);
...
try {
    if ($idUser !== null) {
        $linked = medidata_staff_id_user_linked($connect, $idUser, 'nurse');
        if ($linked !== null) {
    ...
    $check = $connect->prepare('SELECT COUNT(*) FROM nurse WHERE numide = :numide');
    ...
    if ($num_empleado === '') {
        $stmtC = $connect->query("SELECT COUNT(*) FROM nurse");
    ...
    $stmt = $connect->prepare('
        INSERT INTO nurse (
            id_user, numide, nomnur, apenur, nacinur, sexnur, state,
    ...
    if ($ok) {
        $returnPage = medidata_staff_return_page($_POST, 'enfermera.php');
        echo '<script>Swal.fire("Agregado", "Enfermero(a) registrado correctamente", "success").then(function(){ window.location=' . json_encode($returnPage, JSON_UNESCAPED_UNICODE) . '; });</script>';
    } else {
        echo '<script>Swal.fire("Error", "No se pudo registrar el colaborador", "error");</script>';
    }
} catch (Throwable $e) {
    error_log('add_nurse: ' . $e->getMessage());
