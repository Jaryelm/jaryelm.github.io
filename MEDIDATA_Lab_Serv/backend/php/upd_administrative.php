...
try {
    ...
    $stmtCurrent = $connect->prepare("SELECT numide, id_candidate_rrhh FROM staff_administrative WHERE idadm = ? LIMIT 1");
    $stmtCurrent->execute([$idadm]);
    ...
    $updates = [
        'id_user = :id_user', 'numide = :numide', 'nomadm = :nomadm', 'apeadm = :apeadm',
        'nacadm = :nacadm', 'sexadm = :sexadm', 'num_empleado = :num_empleado',
    ...
    $sql = 'UPDATE staff_administrative SET ' . implode(', ', $updates) . ' WHERE idadm = :idadm LIMIT 1';
