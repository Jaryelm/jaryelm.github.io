...
try {
    $stmt = $connect->prepare('UPDATE staff_medifarma SET state = :state WHERE idmf = :id LIMIT 1');
    $ok = $stmt->execute([':state' => (string) $state, ':id' => $id]);
    medidata_link_user_state_from_staff($connect, 'staff_medifarma', 'idmf', $id, $state);
    echo json_encode(['success' => (bool) $ok, 'message' => $ok ? 'Estado actualizado.' : 'No se pudo actualizar el estado.']);
} catch (Throwable $e) {
    error_log('toggle_medifarma_state: ' . $e->getMessage());
