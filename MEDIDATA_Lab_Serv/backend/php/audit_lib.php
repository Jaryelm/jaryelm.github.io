 ...
 * Registra en medic9ue_hr_leaves.hr_absence_audit_log de forma centralizada.
 *
 * Se usa el nombre de tabla CALIFICADO para que funcione tanto con la conexión de
 * ausencias ($connect_hr_leaves) como con la principal ($connect), ya que ambas
 * apuntan al mismo servidor MySQL.
 *
...
if (!function_exists('medidata_audit_log')) {
    /**
     * @param PDO         $pdo             Conexión ($connect o $connect_hr_leaves)
    ...
    function medidata_audit_log(PDO $pdo, ?int $action_user_id, string $action, string $affected_table, $record_id = null, $old_value = null, $new_value = null): void
    {
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO medic9ue_hr_leaves.hr_absence_audit_log
