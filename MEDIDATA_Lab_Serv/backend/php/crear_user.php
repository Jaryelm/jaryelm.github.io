...
if (!function_exists('medidata_ensure_users_cedula_sexo')) {
    function medidata_ensure_users_cedula_sexo(PDO $connect): void
    {
        try {
            $ced = $connect->query("SHOW COLUMNS FROM users LIKE 'cedula'")->fetch(PDO::FETCH_ASSOC);
            if (!$ced) {
                $connect->exec("ALTER TABLE users ADD COLUMN cedula VARCHAR(30) NULL DEFAULT NULL");
            }
            $sex = $connect->query("SHOW COLUMNS FROM users LIKE 'sexo'")->fetch(PDO::FETCH_ASSOC);
            if (!$sex) {
...
medidata_ensure_users_cedula_sexo($connect);
...
if ($cedula === '') {
    medidata_crear_user_alert('Campo vacío', 'Por favor ingrese la cédula.', 'warning');
    return;
}
if ($sexo === '') {
    medidata_crear_user_alert('Campo vacío', 'Por favor seleccione el sexo.', 'warning');
    return;
}
if (strlen($cedula) > 30) {
    medidata_crear_user_alert('Cédula muy larga', 'La cédula no puede superar 30 caracteres.', 'warning');
    return;
}
if ($email === '') {
...
try {
    ...
    $stmt = $connect->prepare(
        'INSERT INTO users (username, name, cedula, sexo, email, password, rol, created_at, uid_biometrico)
         VALUES (:username, :name, :cedula, :sexo, :email, :password, :rol, :created_at, :uid_biometrico)'
    );
    $stmt->execute([
        ':username' => $username,
        ':name' => $nombreCompleto,
