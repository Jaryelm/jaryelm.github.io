document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchData(searchTerm = '') {
        const url = '../../backend/registros/tabla_puestos_trabajo.php' +
        ...
        fetch(url, { credentials: 'same-origin' })
            ...
            .then(function (data) {
                if (!Array.isArray(data)) {
                    if (data && data.error) {
                        ...
                        showFetchError('No se pudo cargar el listado de puestos.');
                    }
                    return;
                }
                renderCards(data);
            ...
            .catch(function (error) {
                ...
                showFetchError(error && error.message ? error.message : 'Error al cargar puestos de trabajo');
            });
    }
    ...
    function showFetchError(message) {
        ...
        gridContainer.innerHTML = `
            <div style="grid-column: 1 / -1; width: 100%;">
                <p class="alert alert-warning" style="margin: 0; display: block; width: 100%; border-radius: 5px; text-align: left;">
                    ${escapeHtml(message || 'Error al cargar puestos de trabajo')}
                </p>
            </div>
        `;
    }
    ...
    function attachStatusToggles() {
        document.querySelectorAll('.status-toggle').forEach(toggle => {
            toggle.addEventListener('change', function() {
                ...
                fetch('../../backend/php/toggle_puesto_trabajo.php', {
                    method: 'POST',
                ...
                .then(resp => {
                    ...
                    label.textContent = deleted == 0 ? 'Activo' : 'Inactivo';
                })
                .catch(() => {
                    self.checked = !self.checked;
