...
try {
    ...
    // Asegurar columnas cedula/sexo en users
    try {
        $stmtCed = $connect->query("SHOW COLUMNS FROM users LIKE 'cedula'");
        if (!$stmtCed || !$stmtCed->fetch()) {
            $connect->exec("ALTER TABLE users ADD COLUMN cedula VARCHAR(30) NULL DEFAULT NULL");
        }
        $stmtSex = $connect->query("SHOW COLUMNS FROM users LIKE 'sexo'");
        if (!$stmtSex || !$stmtSex->fetch()) {
            $connect->exec("ALTER TABLE users ADD COLUMN sexo VARCHAR(20) NULL DEFAULT NULL");
        }
    } catch (Throwable $e) {
        error_log('Error asegurando columnas cedula/sexo en users: ' . $e->getMessage());
    }
    ...
    $sql = "SELECT idodc AS ID, idodc AS RealID, ceddoc AS Cedula, nodoc AS Nombre,
            ...
            UNION ALL
            SELECT u.id AS ID, u.id AS RealID,
                   COALESCE(NULLIF(u.cedula, ''), 'N/A') AS Cedula, u.name AS Nombre,
                   'Usuario' AS Tipo_Empleado,
                   COALESCE(NULLIF(u.sexo, ''), 'N/A') AS Sexo, u.rol AS Especialidad, u.state AS Estado
            FROM users u
