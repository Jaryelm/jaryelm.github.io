    ...
    function medidata_rrhh_match_aplica_a_vacante(string $puestoAspirado): array
    {
        ...
        try {
            ...
            if (!$vacantes && $result['match_type'] === 'fuzzy') {
                ...
                $sqlLike = "SELECT v.id, p.name AS position_name, v.priority, v.end_date
                            FROM vacant_positions v
                            INNER JOIN positions_details pd ON v.id_position = pd.id
                            INNER JOIN {$mainDb}.positions p ON pd.id_positions = p.id
                            WHERE v.deleted = 0
            ...
            if (count($vacantes) === 1) {
                ...
                $result['suggested_label'] = (string) ($vacantes[0]['position_name'] ?? 'Vacante');
    ...
    function medidata_rrhh_incorporar_aplica(int $idAplica, int $idVacante, string $usuario): array
    {
        ...
        try {
            $stmtVac = $pdoRrhh->prepare(
                "SELECT v.id FROM vacant_positions v
