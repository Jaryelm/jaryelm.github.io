 ...
 */
    public function __construct(string $ip, int $port = 4370)
    {
        ...
        // Tiempo máximo por socket_recvfrom (Linux respeta bien; en Windows+XAMPP un valor
        // alto congela el script hasta max_execution_time). Medicasa: configurable vía getenv.
        $sec = getenv('MEDIDATA_ZK_RECV_SEC');
        $timeoutSec = (is_numeric($sec) && (int) $sec > 0)
            ? min(30, max(1, (int) $sec))
            : 8;
    ...
    public function _command($command, $command_string, $type = Util::COMMAND_TYPE_GENERAL)
    {
        ...
        try {
            $recvAddr = '';
            $recvPort = 0;
            $bytes = socket_recvfrom($this->_zkclient, $this->_data_recv, 1024, 0, $recvAddr, $recvPort);
 ...
 */
    ...
    {
        // Attendance::get() only accepts ZKTeco; limit is applied locally (newest first).
        $limit = max(1, (int) $limit);
        $attendance = Attendance::get($this);
        if ($attendance === []) {
            return [];
        }
        ...
        return array_slice($attendance, 0, $limit);
    }
