...
class Util
{
  ...
  static public function recData(ZKTeco $self, $maxErrors = 10, $first = true)
  {
    ...
    // Tamaño declarado absurdamente alto (cabecera corrupta / equipo desincronizado)
    $maxDeclare = 8 * 1024 * 1024; // 8 MiB — más que suficiente para listas grandes
    if ($bytes > $maxDeclare) {
      self::logger($self, 'recData: declared size too large (' . $bytes . ' bytes), aborting');
      return '';
    }

    // Límite de reloj por bloque UDP (GET attendance + GET users pueden llamar dos veces seguidas).
    $secRaw = getenv('MEDIDATA_ZK_RECDATA_MAX_SEC');
    $maxWall = (is_numeric($secRaw) && (float) $secRaw > 0)
        ? min(120.0, max(5.0, (float) $secRaw))
        : 40.0;
    $deadline = microtime(true) + $maxWall;

    $received = 0;
    $errors = 0;
    $maxIterations = (int) max(10000, ceil($bytes / 400) + 1000);
    $iterations = 0;

    while ($bytes > $received) {
      $iterations++;
      if ($iterations > $maxIterations || microtime(true) > $deadline) {
        self::logger($self, 'recData: wall/iteration limit; got ' . $received . ' of ' . $bytes . ' bytes');
        return '';
      }
      ...
      $recvPort = 0;
      $dataRec = '';
      $ret = @socket_recvfrom($self->_zkclient, $dataRec, 1032, 0, $recvAddr, $recvPort);

      // false = error/timeout; 0 = sin datos útiles (evita bucle infinito)
      if ($ret === false || $ret < 1) {
        if ($errors < $maxErrors) {
          $errors++;
          usleep(250000); // 250ms (antes sleep(1) sumaba tiempo de página)
          continue;
        }
    ...
    $recvPort = 0;
    $dataRec = '';
    @socket_recvfrom($self->_zkclient, $dataRec, 1024, 0, $recvAddr, $recvPort);
    unset($dataRec);
