...
if (isset($_POST['add_medicine'])) {
    try {
        ...
        $stock_minimo = trim($_POST['stock_minimo'] ?? 5);
        ...
        if (!in_array($impuesto, ['G', 'E'], true)) {
            echo '<script>
                Swal.fire("Error", "Debe seleccionar un impuesto válido (Gravado o Exento).", "error")
                .then(function() { window.location = "nuevo.php"; });
                </script>';
            exit;
        }
        ...
        if (isset($_FILES['adj_foto']) && $_FILES['adj_foto']['error'] === UPLOAD_ERR_OK) {
            ...
            // Sanear nombre del archivo: el JS de la lista de inventario
            // (tabla_almacen.js) elimina con la regex /[^a-zA-Z0-9._\-\/]/g
            // espacios, tildes y ñ al construir la URL. Si dejáramos el nombre
            // original con espacios/acentos, la URL no coincidiría con el
            // archivo en disco y la imagen aparecería rota (alt="Foto").
            ...
            // Ruta absoluta robusta: en producción y desarrollo cambia el path,
            // por lo que se calcula desde la ubicación del propio script.
        ...
        $stmt_insert = $connect->prepare("INSERT INTO product(
        ...
        $stmt_insert->bindParam(':stock_minimo', $stock_minimo);
